...
- `2024 세분류 토지피복`과 `S-DoT 온도센서` 두 개의 단순한 레이어 토글
- S-DoT 포인트와 300m 버퍼를 하나의 `S-DoT 온도센서` 레이어로 통합
- 300m 버퍼에 마우스를 올리면 선택한 분류단계의 구성비 파이차트 표시
...
- 시가화건조지역 비율 10%p 증가에 따른 온도 변화를 추정하는 OLS 회귀분석과 95% 신뢰구간 그래프
- 센서표 CSV 다운로드
...
- 상관계수와 온도차는 탐색적 연관성이며 인과관계를 의미하지 않습니다.

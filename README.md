...
- 고온일수록 주황색이 강해지고 선택 피복비율이 높을수록 짙어지며, 두 값이 모두 높을 때 가장 진해지는 3×3 이변량 관계 지도
...
- 시가화건조 비율-녹지 비율로 만든 도심성 지수의 산지 인접형·전이지대·도심부 평균 온도와 95% 신뢰구간 도표
...
- 이변량 지도는 각 변수를 삼분위로 나눠 9가지 조합 색상으로 표시합니다. 온도가 높을수록 주황색이 강해지고 선택 피복비율이 높을수록 짙어지며, 두 값이 모두 높을 때 가장 진한 주황색으로 표시됩니다.
- 도심성 지수는 시가화건조지역 비율 - 녹지 비율이며 실제 산지까지의 거리가 아닌 토지피복 기반 탐색 지표입니다.
